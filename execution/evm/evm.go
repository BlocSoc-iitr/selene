...
func (e *Evm[EXT, DB]) TransactPreverifiedInner(initialGasSpend uint64) EVMResult[DatabaseError] {
	...
	ctx = &e.Context
	_= e.Handler.Execution.LastFrameReturn(ctx, &result)
...
func (e *Evm[EXT, DB]) RunTheLoop(firstFrame Frame) (FrameResult, EvmError) {
	...
	for {
		...
		case Result_FrameOrResult:
			...
			case Call:
				if err := exec.InsertCallOutcome(ctx, stackFrame, &sharedMemory, *frameOrResult.Result.Call); err.Message != "" {
					return FrameResult{}, EvmError{Message: err.Error()} // handle the error appropriately
				}
			case Create:
				if err := exec.InsertCreateOutcome(ctx, stackFrame, *frameOrResult.Result.Create); err.Message != "" {
					return FrameResult{}, EvmError{Message: err.Error()} // handle the error appropriately
				}
			case EOFCreate:
				if err := exec.InsertCreateOutcome(ctx, stackFrame, *frameOrResult.Result.Create); err.Message != "" {
